...
void loop()
{
  ...
  while (team1Score < WINNING_SCORE && team2Score < WINNING_SCORE)
  {
    ...
    //Length of each round is between 30 and 60 seconds
    int roundTimeLimit = random(30,60);

    //Display the first word
    displayNextWord(wordList, numWords);

    //Timer is running, word to guess is displayed on screen
    while (gameClock < roundTimeLimit)
    {
...
void initSDCard()
{
  ...
  if (cardType == CARD_NONE)
  {
    ...
    printToScreen("No SD card attached", 0, 0);
    return;
  }
...
void displayNextWord(String wordList[], int size)
{
  ...
  formattedLCDDisplay(wordList[index]);
...
void formattedLCDDisplay(String word)
{
  if (word.length() <= 11)
  {
